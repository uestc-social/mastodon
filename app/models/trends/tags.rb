...
class Trends::Tags < Trends::Base
  ...
  self.default_options = {
    threshold: ENV.fetch('TRENDS_TAGS_THRESHOLD', ENV.fetch('TRENDS_THRESHOLD', 5)).to_i,
    review_threshold: ENV.fetch('TRENDS_TAGS_REVIEW_THRESHOLD', ENV.fetch('TRENDS_REVIEW_THRESHOLD', 3)).to_i,
    max_score_cooldown: TrendsSettingsParser.parse_duration(ENV.fetch('TRENDS_TAGS_MAX_SCORE_COOLDOWN', ENV.fetch('TRENDS_MAX_SCORE_COOLDOWN', nil)), 2.days).freeze,
    max_score_halflife: TrendsSettingsParser.parse_duration(ENV.fetch('TRENDS_TAGS_MAX_SCORE_HALFLIFE', ENV.fetch('TRENDS_MAX_SCORE_HALFLIFE', nil)), 4.hours).freeze,
    decay_threshold: ENV.fetch('TRENDS_TAGS_DECAY_THRESHOLD', ENV.fetch('TRENDS_DECAY_THRESHOLD', 1)).to_i,
  }
